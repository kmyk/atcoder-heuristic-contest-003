...
vector<pair<int, int>> solve_with_scan(int sy, int sx, int ty, int tx, const array<bool, H>& rows) {
    ...
    bool is_right = false;
    path.go_col(W / 2 - SCAN_R - 1);
    REP (y, H) {
        if (rows[y]) {
            path.go_row(y);
            path.go_col(is_right ? W / 2 - SCAN_R : W / 2 + SCAN_R - 1);
...
vector<pair<int, int>> solve_with_scan_hr(int sy, int sx, int ty, int tx, int k, RandomEngine& gen) {
    assert (0 <= k and (2 << k) <= 2 * SCAN_R);
    array<bool, H> rows = {};
    REP (y, 2 * SCAN_R) {
        rows[H / 2 - SCAN_R + y] = (y % (2 << k) < (1 << k));
    }
...
template <class RandomEngine>
vector<pair<int, int>> solve_with_scan_vr(int sy, int sx, int ty, int tx, int k, RandomEngine& gen) {
    auto path = solve_with_scan_hr(sx, sy, tx, ty, k, gen);
    REP (i, path.size()) {
...
void solve(function<tuple<int, int, int, int> ()> read, function<int64_t (const string&)> write, int K, RandomEngine& gen, chrono::high_resolution_clock::time_point clock_end) {
    ...
    vector<pair<bool, int>> scan;
    REP (is_row, 2) {
        REP (k, 4) {
            scan.emplace_back(is_row, k);
            scan.emplace_back(is_row, k);
        }
    }
    ...
    REP (query, K) {
        ...
        vector<pair<int, int>> path;
        REP (j, scan.size()) {
            auto [is_row, k] = scan[j];
            if (is_row) {
                path = solve_with_scan_hr(sy, sx, ty, tx, k, gen);
            } else {
                path = solve_with_scan_vr(sy, sx, ty, tx, k, gen);
            }
            if (not path.empty()) {
